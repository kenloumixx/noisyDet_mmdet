...
class StandardRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def _bbox_forward_gmm(self, x, sampling_results, gt_bboxes, gt_labels,
        ...
        # # -> sampling_results에서 어떻게 변하는지 확인하기

        # # 여기에 gmm label 넣기. <- get_targets에서!
        # bbox_targets = self.bbox_head.get_targets_splitnet(sampling_results, gt_bboxes,
        #                                           gt_labels, self.train_cfg)
        # # loss_bbox = self.bbox_head.loss(bbox_results['cls_score'],
        # #                                 bbox_results['bbox_pred'], rois,
        # #                                 *bbox_targets)
        # cls_loss, cls_labels, bbox_loss, logits = self.bbox_head.gmm_los(bbox_results['cls_score'],
        #                        bbox_results['bbox_pred'], rois, box_ids, sampling_results.pos_assigned_gt_inds, *bbox_targets)
    ...
    def gmm_simple_test(self,
        ...
        if self.with_bbox or self.with_mask:
            ...
            if gt_bboxes_ignore is None:
                gt_bboxes_ignore = [None for _ in range(num_imgs)]
            sampling_results = []
            for i in range(num_imgs):   #이전에 이미 나옴
                assign_result = self.bbox_assigner.assign_gmm(
                    proposal_list[i], gt_bboxes[i], gt_bboxes_ignore[i],
                    gt_labels[i], box_ids[i])   # assign results가 output으로 나옴
                # AssignResult의 object -> assigned_gt_inds를 찾아보기!
                sampling_result = self.bbox_sampler.sample(
                    assign_result,
                    proposal_list[i],
                    gt_bboxes[i],
        ...
        # bbox head forward and loss    # loss_bbox     <- 여기에 GT 구역의 loss값만 가져오기
        cls_loss, cls_labels, bbox_loss, logits = self._bbox_forward_gmm(x, sampling_results,
                                                gt_bboxes, gt_labels,
                                                img_metas)
        # # 음..
        # bbox_idx_for_pos_bbox = sampling_result.pos_assigned_gt_inds  # torch.tensor

        return box_ids[0], cls_loss, cls_labels, bbox_loss, logits
